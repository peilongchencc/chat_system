...
class ChineseRecursiveTextSplitter(RecursiveCharacterTextSplitter):
    ...
    def __init__(
            ...
            is_separator_regex: bool = True,        # 分隔符是否为正则表达式，RecursiveCharacterTextSplitter(父类为 TextSplitter)默认是不启用re的。
            **kwargs: Any,
    ) -> None:
    ...
    def _split_text(self, text: str, separators: List[str]) -> List[str]:
        """重写了RecursiveCharacterTextSplitter(父类为 TextSplitter)中的_split_text方法，分割传入的文本并返回文本块。"""
        ...
        for s in splits:
            if self._length_function(s) < self._chunk_size:
                ...
                if not new_separators:
                    ...
                    # 如果当前文本块过长且还有其他分隔符可用，则调用自身，递归使用其他分隔符进行分割。
...
def get_documents(filepath):
    ...
    # 使用加载器加载文档
    texts = loader.load()   # 数据类型为list [(page_content='（一）直接打压式\n洗盘\n直接打压较多出现在 庄家 吸货区域，目的是... metadata={'source': 'example_data.txt'}' metadata={'source': 'example_data.txt'})]
    documents = []
    if texts is not None:
        texts = text_splitter.split_documents(texts)    # split_documents内部会调用create_documents，最终的实效效果还是取决于create_documents。
